...
#시도별 국내 현황
region_url = 'http://ncov.mohw.go.kr/bdBoardList_Real.do?brdId=1&brdGubun=13&ncvContSeq=&contSeq=&board_id=&gubun='
...
def regionTable():
    # 기본 테이블 틀 생성
    column_name = ['UPD_DATE','SIDO','SIGUN','TOT_NUM','TODAY(TOT)','TODAY(DOM)','TODAY(INTL)',
                   'RLSE(TOT)','RLSE(NEW)','QUAR(TOT)','QUAR(NEW)','DTH(TOT)','DTH(NEW)']
    table_region = pd.DataFrame(columns=column_name)

    # 질본 테이블 전처리
    raw_table = pd.read_html(str(region_soup.find('table',{'class':'num midsize'})))[0]
    raw_table.columns = ['SIDO','TODAY(TOT)','TODAY(DOM)','TODAY(INTL)','TOT_NUM','QUAR(TOT)','RLSE(TOT)','DTH(TOT)','nonuse']
...
#국내 누적 확진자 현황
def domesticTable():
    domestic_url = 'http://ncov.mohw.go.kr/bdBoardList_Real.do?brdId=1&brdGubun=11&ncvContSeq=&contSeq=&board_id=&gubun='
    domestic_soup = requestsSoup(domestic_url)

    #기본 테이블 틀 생성
    column_name = ['UPD_DATE','TOT_NUM','TODAY(TOT)','TODAY(DOM)','TODAY(INTL)','RLSE(TOT)','RLSE(NEW)','QUAR(TOT)','QUAR(NEW)','DTH(TOT)','DTH(NEW)']
