...
if latest_standard_date == dt.today().date():
    ...
    select_latest_data_query = f"""
                                SELECT alpha_2_code, confirmed_total, deceased_total
    ...
    latest_df_confirmed_total_dict = (
        latest_df[["alpha_2_code", "confirmed_total"]].set_index("alpha_2_code").T.to_dict("records")[0]
    )
    full_df_groupby_iso2_confirmed_total_dict = (
        full_df_groupby_iso2[["alpha_2_code", "confirmed_total"]]
        .set_index("alpha_2_code")
        .T.to_dict("records")[0]
    )
    ...
    full_df_groupby_iso2["confirmed_daily"] = full_df_groupby_iso2["alpha_2_code"].map(confirmed_total_counter)
    ...
    latest_df_deceased_total_dict = (
        latest_df[["alpha_2_code", "deceased_total"]].set_index("alpha_2_code").T.to_dict("records")[0]
    )
    full_df_groupby_iso2_deceased_total_dict = (
        full_df_groupby_iso2[["alpha_2_code", "deceased_total"]]
        .set_index("alpha_2_code")
        .T.to_dict("records")[0]
    )
    ...
    full_df_groupby_iso2["deceased_daily"] = full_df_groupby_iso2["alpha_2_code"].map(deceased_total_counter)
