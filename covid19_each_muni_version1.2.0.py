...
def clean_update_date(date_element):
    update_date_text_list = re.findall('\d+.|오전|오후',date_element.text)
...
def extract_sigungu_name(target_data_html, tag_for_name, sido_kr):
    ...
    if tag_for_name.startswith('img'):
        ...
        sigungu_name_list = [data['alt'] for data in data_dummy if not data['alt'].endswith(junky_text_pattern) and len(data['alt']) > 2]
        return sigungu_name_list
    ...
    sigungu_name_list = [o for data in data_dummy if (o := filter_data(data, junky_text_pattern))] # 바다코끼리 연산자(the walrus operator)
    return sigungu_name_list
...
def extract_covid19_confirmed(target_data_html, tag_for_covid19, covid19_count_start_idx, sigungu_name_list):
    ...
    raw_covid19_confirmed_count_list = raw_covid19_confirmed_count_list[covid19_count_start_idx:covid19_count_start_idx+len(sigungu_name_list)]
...
for sido in sido_list:
    try:
        ...
        covid19_confirmed_list = extract_covid19_confirmed(target_data_html, tag_for_covid19, covid19_count_start_idx, sigungu_name_list)

        covid19_confirmed_by_sigungu = pd.DataFrame(columns=['SIDO', 'SIGUN', 'TOT_NUM', 'UPD_DATE'])
        covid19_confirmed_by_sigungu['SIGUN'] = sigungu_name_list
        covid19_confirmed_by_sigungu['TOT_NUM'] = covid19_confirmed_list
        covid19_confirmed_by_sigungu['SIDO'] = sido_kr
        covid19_confirmed_by_sigungu['UPD_DATE'] = update_date

        update_table(covid19_confirmed_by_sigungu, dbtable, sido_kr)
