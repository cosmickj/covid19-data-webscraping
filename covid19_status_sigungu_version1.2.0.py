...
def check_sigungun_update(db_table, sido):
    """해당 시도 데이터 베이스 업데이트 날짜 확인"""
    sql = f"SELECT MAX(upd_date) FROM {db_table} WHERE SIDO = '{sido}';"
    sql_df = pd.read_sql(sql, con=engine)
    latest_update_date = sql_df.iloc[0, 0].date()
    return latest_update_date


def update_table(table, db_table, sido):
    table.to_sql(name=f"{db_table}", con=engine.connect(), if_exists="append", index=False)
...
def extract_sigungu_name(target_data_html, tag_for_name, sido_kr):
    if tag_for_name == None:
        ...
        if sido_kr == "세종":
            sido_kr = sido_kr + "시"  # database 업데이트 시 삭제
...
for sido in sido_list:
    try:
        db_table = "covid19_kr_by_Municipality"
        sido_kr = sido_data[sido]["sido_kr"]

        db_sigungu_latest_update_date = check_sigungun_update(db_table, sido_kr)
        ...
        covid19_confirmed_list = extract_covid19_confirmed(
            target_data_html, tag_for_covid19, covid19_count_start_idx, sigungu_name_list
        )
        ...
        update_table(covid19_confirmed_by_sigungu, db_table, sido_kr)
